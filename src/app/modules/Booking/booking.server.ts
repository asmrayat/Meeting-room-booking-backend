...
const createBookingIntoDB = async (bookingData: Tbooking) => {
  ...
  bookingData.totalAmount =
    (room?.pricePerSlot as number) * bookingData.slots.length;
  ...
  return (
    await (await result.populate('user', '-password')).populate('slots')
  ).populate('room');
};
...
const getUserBookingFromDB = async (email: string) => {
  ...
  const result = await Booking.findOne({ user: userData })
    .populate('slots')
    .populate('room')
    .select('-user');
...
const updateSingleBookingInDB = async (
  _id: string,
